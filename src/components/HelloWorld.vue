...
export default {
    ...
    this.$axios
      ...
      .then(res => {
        ...
        for(var i=0;i<this.charList.length;i++){
          this.charList[i]['selected'] = false;
        }
        this.changeClass();
      })
      .catch(function(res) {
  ...
  methods: {
    //删除人员 获取新的数组
    deletePeople() {
      // console.log("charList", this.charList[this.replaceIndex]);
      // this.charList.splice(this.replaceIndex, 1);
      let data = {
        confSeat: this.charList[this.replaceIndex],
        confSeats: this.newPersons,
        id: 1
      };
      this.$axios.getDeleteList(data).then(res => {
        this.newPersons = res.data.data.confSeats;
        // this.deleteList = [];
        this.sortPerson();
      });
    },
      ...
      if (app.flag1 == 2) {
        if (JSON.stringify(app.lastXinxi) == "[]") {
          ...
          for (var i = 0; i < app.xinXinxi.length; i++) {
            ...
            app.sortMessage.push(app.xinXinxi[i]);
            app.sortIndex.push(app.xinSeat[i]);
          }
        ...
        for (var j = 0; j < this.xinXinxi.length; j++) {
          ...
          if (i == this.xinSeat[j]) {
            ...
            if (dataType == 1) {
              ...
              this.xinXinxi[j].orderMark = null;
              if (oT1[i].innerText != "占位") {
                if (this.option[this.xinIndex[j]] != null) {
                  oT1[i].innerText = this.option[this.xinIndex[j]];
                  this.xinXinxi[j].orderMark = this.option[this.xinIndex[j]];
    ...
    insertPeople() {
      ...
      this.$axios
        ...
        .then(res => {
          ...
          if(this.length != 0){            
            for(var j = this.length;j<this.persons.length;j++){
              this.newPersons.push(this.persons[j]);
            }
                ...
                for (let i of this.idxList) {
                    for (let j in this.newPersons) {
                        if (this.charList[i].id == this.newPersons[j].id) {
                            this.charList[i].userUnit = this.newPersons[j].userUnit;
                            //删除人员列表
                            if (this.newPersons[j].userUnit != null) {
                                a++;
                            }
                        }
                    }
